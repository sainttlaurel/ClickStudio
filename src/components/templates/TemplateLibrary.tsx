...
import { Search, X, Camera, Grid3X3, Heart, Eye, Sparkles, Check } from 'lucide-react'
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-8 pt-8 pb-5 border-b border-border/30">
              ...
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full bg-rose-50 border border-border/40 flex items-center justify-center hover:bg-rose-100 hover:scale-105 transition-all"
              >
            ...
            <div className="flex items-center gap-3 px-8 pt-5 pb-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full h-12 pl-11 pr-4 rounded-2xl border border-rose-200 bg-white text-sm text-text placeholder:text-muted/40 focus:outline-none focus:ring-[3px] focus:ring-primary/15 focus:border-primary transition-all shadow-sm"
                />
              </div>
              <select
                value={sortBy}
            ...
            <div className="relative px-8 pb-5">
              <div
                className="flex gap-2.5 overflow-x-auto snap-x snap-mandatory py-1"
              ...
              >
                {TEMPLATE_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={cn(
                      'snap-start flex-shrink-0 px-4 py-2.5 rounded-full text-xs font-semibold border transition-all duration-200',
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          <div className={cn('h-24 flex items-center justify-center', cfg?.bg || 'bg-rose-50')}>
                            <span className="text-2xl opacity-20">{template.previewEmoji || '📸'}</span>
                          </div>
                          <div className="bg-white p-2.5">
                            <p className="font-display text-[11px] text-text truncate">{template.name}</p>
                            <p className="text-[9px] text-muted/60">{count} shots</p>
                          </div>
                        </div>
                      </motion.div>
                    )
                        ...
                        <div
                          ...
                          className={cn(
                            ...
                            isSelected
                              ...
                              : 'border-border/50 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.08)] hover:-translate-y-1 hover:border-primary/40'
                          )}
                        >
                          {/* Badge */}
                          {template.badge && (
                          ...
                          {/* Preview */}
                          <div className={cn('h-32 flex items-center justify-center transition-transform duration-300 group-hover:scale-[1.02]', cfg?.bg || 'bg-rose-50')}>
                          ...
                          <div className="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-30">
                            <div className="bg-gradient-to-t from-white via-white/95 to-transparent pt-10 pb-3 px-3 flex items-center justify-center gap-2">
                              ...
                              <button
                                onClick={e => e.stopPropagation()}
                                className="w-7 h-7 rounded-full bg-white border border-border/50 flex items-center justify-center text-muted hover:text-primary hover:border-primary/40 transition-all shadow-sm"
                              >
